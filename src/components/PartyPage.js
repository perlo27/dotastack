...
class PartyPage extends Component {
  handleLeave = ev => {
    ...
    leaveParty(user, party.id)
  }
  render() {

    const {party} = this.props
    console.log(party)
    const players = party.players.map( player => <li key={player.id}>{player.nick}, mmr:{player.mmr}</li>)
    return (
      <div>
...
export default connect((state, { id }) => ({
  party: state.parties.find(party => party.id == id),
  user: state.user
